...
| 6 | `7_prediction.sh` | Download model files from [Hugging Face](https://huggingface.co/zengwenwu/ESM-DBP/tree/main) and the necessary scripts from https://github.com/pengsl-lab/ESM-DBP -> Place the `ESM-DBP.model` files in the `ESM-DBP/` directory and the required scripts accordingly -> Run `./7_prediction.sh "/ESM-DBP/model" "/path/to/the/fasta/files/of/TF" "/path/to/output/directory" "cuda:0"` |
