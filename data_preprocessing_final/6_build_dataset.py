"""Build the DeepSEA dataset with simple chromosome-based splitting."""
...
parser.add_argument("--train_size", type=int, help="Size of the training set. Will be multiplied by 2 if complementary sequence is True")
parser.add_argument("--valid_size", type=int, help="Size of the validation set. Will be multiplied by 2 if complementary sequence is True")
parser.add_argument("--test_size", type=int, help="Size of the test set. Will be multiplied by 2 if complementary sequence is True")
...
# change filter from command line arguments
...
create_test_data = args.test_data_filename or args.test_filename
create_test_labels = args.test_labels_filename or args.test_filename
...
# SIMPLE CHROMOSOME-BASED SPLITTING
# Calculate sizes based on actual chromosome content
test_size = features.no_of_bins([CHR_8, CHR_9])  # All of chr8 and chr9
valid_size = features.no_of_bins([CHR_7])        # All of chr7
train_size = features.no_of_bins() - test_size - valid_size  # Everything else

# Override with explicit sizes if provided
if args.train_size:
    train_size = min(train_size, args.train_size)
if args.valid_size:
    valid_size = min(valid_size, args.valid_size)
if args.test_size:
    test_size = min(test_size, args.test_size)
...
# DEBUGGING INFO
print(f"=== SIMPLE CHROMOSOME SPLITTING ===")
print(f"Total bins: {features.no_of_bins():,}")
print(f"Test bins (chr8+9): {test_size:,}")
print(f"Validation bins (chr7): {valid_size:,}")
print(f"Training bins (rest): {train_size:,}")
print(f"Sum: {train_size + valid_size + test_size:,}")
...
if args.save_debug_info:
    ...
    debug_train = csv.DictWriter(open("debug_train.tsv", "w"), fieldnames=fieldnames, delimiter='\t')
    debug_train.writeheader()
    debug_valid = csv.DictWriter(open("debug_valid.tsv", "w"), fieldnames=fieldnames, delimiter='\t')
    debug_valid.writeheader()
    debug_test = csv.DictWriter(open("debug_test.tsv", "w"), fieldnames=fieldnames, delimiter='\t')
...
# DEBUGGING: Track actual assignments
actual_assignments = {'train': [], 'valid': [], 'test': []}
...
if create_labels or create_data:
    ...
    for chr_no in chr_nos_sorted_by_name():
        for bin_start in sorted(bins[chr_no]):
            ...
            # Test set: ALL of chr8 and chr9
            if chr_no in [CHR_8, CHR_9] and test_idx < test_size:
                actual_assignments['test'].append((chr_no, bin_start))
            ...
            # Validation set: ALL of chr7
            elif chr_no == CHR_7 and valid_idx < valid_size:
                actual_assignments['valid'].append((chr_no, bin_start))
                fill(features, hg19, bin_start, chr_no, args.bin_size, args.window_size, args.complementary_sequence, \
            ...
            # Training set: everything else (chr1-6, chr10-22, chrX, chrY)
            elif chr_no not in [CHR_7, CHR_8, CHR_9] and train_idx < train_size:
                actual_assignments['train'].append((chr_no, bin_start))
                fill(features, hg19, bin_start, chr_no, args.bin_size, args.window_size, args.complementary_sequence, \
            ...
            if assigned and i % 1000 == 0:
                logging.debug(f"{100*i/(train_size+valid_size+test_size):.2f}%")

    # Final assignment summary
    print(f"\n=== ACTUAL ASSIGNMENTS ===")
    print(f"Training: {len(actual_assignments['train']):,} bins")
    print(f"Validation: {len(actual_assignments['valid']):,} bins") 
    print(f"Test: {len(actual_assignments['test']):,} bins")
    print(f"Total processed: {len(actual_assignments['train']) + len(actual_assignments['valid']) + len(actual_assignments['test']):,}")

    # Show which chromosomes ended up where
    train_chrs = set(chr_no for chr_no, _ in actual_assignments['train'])
    valid_chrs = set(chr_no for chr_no, _ in actual_assignments['valid'])
    test_chrs = set(chr_no for chr_no, _ in actual_assignments['test'])

    print(f"\nChromosomes in training: {sorted(train_chrs)}")
    print(f"Chromosomes in validation: {sorted(valid_chrs)}")
    print(f"Chromosomes in test: {sorted(test_chrs)}")
